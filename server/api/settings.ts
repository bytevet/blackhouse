...
import type { AuthEnv, AdminEnv } from "../middleware/auth.js";
...
app.get("/agent-configs", authMiddleware, async (c) => {
  const rows = await db
    .select()
...
app.delete("/agent-configs/:id", adminMiddleware, async (c) => {
...
app.post("/agent-configs/:id/build", adminMiddleware, async (c) => {
...
app.get("/agent-configs/:id/build-status", authMiddleware, async (c) => {
  const id = c.req.param("id");
...
app.get("/docker", adminMiddleware, async (c) => {
...
app.get("/docker/status", adminMiddleware, async (c) => {
  try {
    const docker = await getDockerClient();
...
app.get("/containers", adminMiddleware, async (c) => {
  try {
    const docker = await getDockerClient();
...
app.delete("/users/:id", adminMiddleware, async (c) => {
  const session = c.get("session");
