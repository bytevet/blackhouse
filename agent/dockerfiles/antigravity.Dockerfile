...
# Antigravity inherits Gemini's config directory: `agy` writes to
# `~/.gemini/antigravity-cli/...` and `~/.gemini/config/...` — the binary
# even logs "using hardcoded .gemini" when it can't resolve GeminiDir.
# The named volume is still called `antigravity-config` (identifier-only)
# but it mounts at `~/.gemini` so config + auth persist across restarts.
